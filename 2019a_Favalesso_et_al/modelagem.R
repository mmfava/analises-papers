...
## Rodar os modelos de nicho

# 1. Limpando a memoria novamente
rm(list = ls())
gc()
...
# 2. Importando os dados de diretório
...
#  Importar as variáveis - colocar como diretório o local das variáveis selecionadas
...
# "C:\Users\john01\Documents\R\win-library\3.4\dismo\java"
...
for(i in 1:length(levels(po[, 1]))){ # for to each specie
  ...
  for(r in 1:5){	# number of replicas
    ...
    # 1.2 projection
    writeRaster(predict(en.00k, Bioclim), paste0(AOGCM, "_bioclim_00k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE)	
    writeRaster(predict(en.06k, Bioclim), paste0(AOGCM, "_bioclim_06k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE)
    writeRaster(predict(en.21k, Bioclim), paste0(AOGCM, "_bioclim_21k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE) 
    ...
    # 2.2 projection
    writeRaster(predict(en.00k, Gower), paste0(AOGCM, "_gower_00k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE) 
    writeRaster(predict(en.06k, Gower), paste0(AOGCM, "_gower_06k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE)
    writeRaster(predict(en.21k, Gower), paste0(AOGCM, "_gower_21k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE) 
    ...
    # 3.2 projection
    writeRaster(predict(en.00k, Maha), paste0(AOGCM, "_mahalanobis_00k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE) 
    writeRaster(predict(en.06k, Maha), paste0(AOGCM, "_mahalanobis_06k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE)
    writeRaster(predict(en.21k, Maha), paste0(AOGCM, "_mahalanobis_21k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE) 
    ...
    # 4.2 projection
    writeRaster(predict(en.00k, Maxent), paste0(AOGCM, "_maxent_00k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE) 
    writeRaster(predict(en.06k, Maxent), paste0(AOGCM, "_maxent_06k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE)
    writeRaster(predict(en.21k, Maxent), paste0(AOGCM, "_maxent_21k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE) 
    ...
    # 5.2 projection
    writeRaster(predict(en.00k, SVM), paste0(AOGCM, "_svm_00k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE) 
    writeRaster(predict(en.06k, SVM), paste0(AOGCM, "_svm_06k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE)
    writeRaster(predict(en.21k, SVM), paste0(AOGCM, "_svm_21k_", id.specie, ifelse(r < 10, paste0("0", r), r), ".tif"), format = "GTiff", overwrite=TRUE) 
